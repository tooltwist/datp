...
class i2iBackend_WalletTopupStep extends Step {
  ...
  async invoke(instance) {
    if (VERBOSE) {
      instance.console(`i2iBackend_WalletTopupStep (${instance.getStepId()})`)
    }
    ...
    try {
      const url = 'https://api.stg.i2i.ph/api-apic/wallet/topup/process'
      ...
      if (reply.status !== 200) {
        console.log(`\n\n\n ********** ERROR RETURN\n\n`)
        console.log(`The topup failed with status ${reply.response.status}`)
        console.log(`\n\n\n ********** ERROR RETURN\n\n`)
        return instance.finish(Step.FAIL, 'Topup request failed', reply.data)
      }
      ...
      console.log(`\n\n\n ********** EXCEPTION RETURN\n\n`)
      instance.finish(Step.FAIL, 'Topup request failed', e.response.data)
    }
  }//- invoke
}//- class

async function register() {
  await StepTypes.register(myDef, 'i2iBackend_WalletTopUp', 'Top up wallet balance')
}//- register
...
async function factory(definition) {
  const obj = new i2iBackend_WalletTopupStep(definition)
  // console.log(`obj=`, obj)
  return obj
}//- factory
